...
query_input = st.text_input("검색어", value="", placeholder="여러 개일 경우 콤마(,)로 구분 (예: 파이썬 강좌, 머신러닝)")
            ...
            with tabs[i]:
                try:
                    with st.spinner(f"'{query}' 검색 중..."):
                        res = fetch_search_api("blog", query, display=display, sort=sort_option)
                        items = res.get("items", [])
                        if not items:
                            st.info("검색 결과가 없습니다.")
                        else:
                            for item in items:
                                ...
                                st.write(f"**블로거**: {item['bloggername']} | **작성일**: {item['postdate']}")
                                st.write(desc)
                                st.divider()
